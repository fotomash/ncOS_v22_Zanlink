...
class ProcessingConfig:
    ...
    use_tick_volume_for_volume_indicators: bool = True  # New option
...
class TechnicalIndicatorEngine:
    ...
    def _calculate_trend_indicators_safe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Aroon - with dynamic column detection
        try:
            ...
            if isinstance(aroon, pd.DataFrame) and len(aroon.columns) > 0:
                ...
                # Try multiple patterns to find the right columns
    ...
    def _calculate_structure_indicators_safe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            # Williams Fractals - looking for 5-bar patterns
            ...
            # Basic Market Structure Analysis (fallback if SMC not available)
            ...
            if SMC_AVAILABLE and len(df) > 20:  # Need minimum data for SMC analysis
                try:
                    ...
                    if structure_points:
                        # Create a more detailed structure column
                        ...
                        # Forward fill SMC structure for continuity
    ...
    def process_file(self, file_path: Path) -> Dict[str, pd.DataFrame]:
        ...
        try:
            ...
            for timeframe in self.config.timeframes:
                ...
                # Resample
                resampled = self._resample_ohlcv(df, timeframe)

                # Skip if not enough data
                if len(resampled) < 2:
                    logger.warning(f"  Skipping {timeframe} - not enough data points ({len(resampled)} rows)")
                    continue
                ...
                enriched = self.indicator_engine.calculate_all_indicators(resampled)
...
def main():
    ...
    config = ProcessingConfig(
        ...
        use_tick_volume_for_volume_indicators=True  # Use tick volume when real volume is 0
    )
